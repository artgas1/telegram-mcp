...
async def _on_new_incoming(event) -> None:
    """Telethon handler: record incoming private (non-bot, non-self) messages
    so the debounce tools can deliver a settled burst."""
    try:
...
async def _main() -> None:
    try:
        ...
        # Register the event-driven incoming-message handler (debounce support)
        client.add_event_handler(_on_new_incoming, _events.NewMessage(incoming=True))
